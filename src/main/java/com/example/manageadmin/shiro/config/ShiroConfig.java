package com.example.manageadmin.shiro.config;
...
import com.example.manageadmin.shiro.JwtFilter;
import com.example.manageadmin.shiro.JwtRealm;
...
//        
//        // 项目图片访问接口 - 需要认证（通过 API 接口访问）
//        filterChain.put("/api/files/projects/**", "jwt");
//  
//        // 静态资源 - 不需要认证
...
//        filterChain.put("/", "anon");
    	...
    	// 公开接口 - 不需要认证
        for (Map<String, String> item : jwtProperties.getFilterChainPublic()) {
        	filterChain.put(item.get("path"), item.get("delimited"));
        }
        
    	// 需要认证
        for (Map<String, String> item : jwtProperties.getFilterChainRequired()) {
        	filterChain.put(item.get("path"), item.get("delimited"));
        }
        ...
        shiroFilter.setUnauthorizedUrl(jwtProperties.getUnauthorizedUrl());
        // 登陆跳转（API 返回 JSON，不跳转页面）
        shiroFilter.setLoginUrl(jwtProperties.getLoginUrl());
